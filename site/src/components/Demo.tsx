...
import { useState, useEffect, useDeferredValue, useLayoutEffect, useRef, useCallback, useId, Fragment } from "react"
	...
	return (
		<div className="w-full">
			...
			<div className="relative pb-8">
				...
				>
					{/* eslint-disable-next-line react-hooks/refs -- the only ref write below is
					    `wordRefs.current[i] = el` inside a callback ref, which React invokes at
					    commit, not during render. The rule cannot prove that and flags the whole
					    map. Verified: no ref `.current` is read in the render path here. */}
					{WORDS.map((word, i) => {
						// Show dwell progress bar on the word currently being dwelled on
						const isDwelling = dwellMode && dwellingIdx === i && dwellProgress > 0
						return (
							// Space interleaved here rather than by a trailing .reduce over the
							// mapped elements: that passed ref-carrying JSX into another function
							// during render, which react-hooks/refs flags.
							<Fragment key={i}>
								{i > 0 && ' '}
								<span
